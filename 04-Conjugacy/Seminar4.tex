...
Если в задаче линейного программирования на минимум допустимое множество непусто и целевая функция ограничена на нём сннизу, то задача имеет решение.
\end{block}
...
$\bA\bx = \mathbf{b}, \; \bx \geq 0$~--- $\mathbf{b}$ лежит в конусе, натянутым на столбцы матрицы $\bA$

$\bp\bA \geq 0, \; \langle \bp, \mathbf{b} \rangle < 0$~--- существует разделяющая гиперплоскость между вектором $\mathbf{b}$ и конусом из столбцов матрицы $\bA$.
\end{block}
\end{frame}
