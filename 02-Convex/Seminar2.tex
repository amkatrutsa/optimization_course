...
\small
\begin{block}{Афинное множество}
Множество $A$ называется афинным, если для любых $x_1$, $x_2 \in A$ и $\theta \in \bbR$ точка $\theta x_1 + (1 - \theta)x_2 \in A$.
\end{block}
Примеры: $\bbR^n$, гиперплоскость, точка.
\begin{block}{Афинная комбинация точек}
Пусть $x_1, \ldots, x_k \in G$, тогда точка $\theta_1 x_1 + \ldots + \theta_k x_k$ при $\sum\limits_{i=1}^k \theta_i = 1$ называется афинной комбинацией точек $x_1,\ldots,x_k$.
\end{block}

\begin{block}{Афинная оболочка точек}
Множество $\left\{ \sum\limits_{i=1}^k \theta_i x_i \; | \; x_i \in G, \sum\limits_{i=1}^k \theta_i = 1 \right\}$ называется афинной оболочкой множества $G$ и обозначается \textbf{aff}(G).
\end{block}
\end{frame}
...
Множество является афинным тогда и только тогда, когда в него входят все афинные комбинации его точек.
\end{block}

\begin{block}{Утверждение 2}
Множество является афинным тогда и только тогда, когда его можно представить в виде $\{\bx | \bA\bx = \mathbf{b} \}$.
\end{block}
\end{frame}
...
\item Образ афинного отображения выпуклого множества~--- выпуклое множество
