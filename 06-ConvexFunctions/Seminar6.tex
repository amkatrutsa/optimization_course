...
или в бесконечномерном случае: $p(\mu) \geq 0$ и $\int\limits p(\mu) = 1$ 
\vspace{-4mm}
\[
f\left( \int\limits_\Omega p(\mu)x(\mu)d\mu \right) \leq \int\limits_\Omega f(x(\mu))p(\mu)d\mu,\ (x(\mu)\in X\ \forall \mu\in\Omega)
\]
